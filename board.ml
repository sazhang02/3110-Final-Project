(* type tile_type = Wall of wall | Pipe of pipe | Empty
   ...
   is 0; (1,0) is 1; (0,1) is 5; (2,2) is 12 *) let index_of_coord dimx
   coord = match coord with (x,y) -> x + dimx * y

   (** [change_to_wall] is the list of tiles that are walls *) let
   change_to_wall dimx walls = List.map (fun _ -> match wall.coord with
   (x, y) -> {tile_type = Wall () (* set to wall *); is_obstacle =
   true}) walls

   (** [replace i t] replaces the tile at index [i] in layout [t] with a
   wall. *) let replace index t = t.(index) <- {tile_type = Wall () (*
   wall *); is_obstacle = true}
